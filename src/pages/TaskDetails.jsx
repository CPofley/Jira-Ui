...
const PRIORITY_CONFIG = {
  LOW: {
	...
	icon: <ChevronDown size={14} className="text-blue-400 font-bold stroke-[3]" />
  },
  MEDIUM: {
	label: 'Medium',
	style: 'bg-amber-950/50 text-amber-300 border-amber-800 font-semibold',
	icon: <Equal size={13} className="text-amber-400 font-bold stroke-[3]" />
  },
  HIGH: {
	label: 'High',
	style: 'bg-orange-950/50 text-orange-300 border-orange-800 font-bold',
	icon: <ChevronUp size={14} className="text-orange-400 font-extrabold stroke-[3]" />
  },
  HIGHEST: {
	label: 'Highest',
	style: 'bg-red-950/50 text-red-300 border-red-800 font-bold',
	icon: <ChevronsUp size={15} className="text-red-400 font-extrabold stroke-[3]" />
  },
  CRITICAL: {
	label: 'Critical',
	style: 'bg-red-900/60 text-red-200 border-red-700 font-extrabold animate-pulse',
	icon: <AlertCircle size={13} className="text-red-400 fill-red-900 font-extrabold" />
  },
  DEFAULT: {
	label: 'Medium',
	style: 'bg-amber-950/50 text-amber-300 border-amber-800 font-semibold',
  ...
  return (
	<div className="min-h-screen w-full bg-slate-900 font-sans text-sm text-slate-100 text-left mb-12 relative">
	  ...
	  <div className="flex max-w-7xl mx-auto px-8 py-6 gap-8">
			  ...
			  <div className="border border-slate-800 rounded-lg divide-y divide-slate-800 overflow-hidden bg-slate-900 shadow-xs">
				{task.subIssues.map((child) => (
				  <div 
					...
					className="flex items-center justify-between p-3 hover:bg-slate-800/80 cursor-pointer transition-colors text-xs group/item gap-4"
				  >
					<div className="flex items-center gap-2.5 min-w-0 flex-1">
					  <span className="font-mono font-bold text-blue-400 group-hover/item:underline flex-shrink-0 w-16">
						TASK-{child.id}
					  </span>
					  ...
					  <span className="text-slate-200 font-medium truncate select-text flex-1" title={child.title}>
						{child.title}
					  </span>
					</div>
		...
		{/* Sidebar Controls (Redesigned with requested field sequence and border behavior) */}
		<div className="w-80 border border-slate-800 rounded-lg p-4 bg-slate-800/60 space-y-4 shadow-sm h-fit relative">
		  
		 {/* TOP ACTION ROW: Compact Status & Delete Button */}
		  <div className="flex items-center justify-between gap-2 border-b border-slate-700/60 pb-3">
			<div className="relative">
			  <select
				...
				className={`w-auto appearance-none bg-transparent border border-transparent hover:border-slate-700 focus:border-blue-500 rounded px-2 py-1 text-xs font-bold tracking-wide outline-none transition-all cursor-pointer ${
				  editingValues.taskStatus === 'DONE' ? 'text-green-400' :
				  editingValues.taskStatus === 'IN_PROGRESS' ? 'text-blue-400' :
				  editingValues.taskStatus === 'BLOCKED' ? 'text-red-400' : 'text-slate-300'
				}`}
			  >
				{(metadata.statuses || []).map(status => (
				  <option key={status} value={status} className="bg-slate-800 text-slate-200 font-medium text-left">{status.replace('_', ' ')}</option>
				))}
			  </select>
			</div>
			{editingValues.taskStatus !== task.taskStatus && !lockedFields.taskStatus && (
			  <button onClick={() => commitFieldUpdate('taskStatus')} className="p-1 bg-green-600 text-white rounded hover:bg-green-700 shadow-sm cursor-pointer"><Check size={12} /></button>
			)}

			<button 
			  onClick={() => setShowDeleteModal(true)} 
			  className="flex items-center justify-center p-1.5 text-red-400 bg-red-950/40 hover:bg-red-900/50 rounded border border-red-900/60 transition-colors cursor-pointer ml-auto"
			  title="Delete Issue"
			>
			  <Trash2 size={13} />
			</button>
		  ...
		  {savingField && <div className="text-[10px] text-blue-400 animate-pulse text-right">saving...</div>}

		  {/* FIELD SEQUENCE 1: ASSIGNEE */}
		  <div>
			<div className="flex justify-between items-center mb-1">
		 ...
		 {/* FIELD SEQUENCE 3: CREATED DATE */}
		  <div>
			...
			<div className="px-1.5 py-1 text-xs text-slate-300 font-medium">
			  {task.createdAt || task.createdDate ? (() => {
				...
				// Manual split fallback for ISO string format (YYYY-MM-DD...)
			...
			<div className="flex items-center gap-2">
			  <div className="relative flex-1">
				<select
				  ...
				  className={`w-full appearance-none rounded px-2.5 py-1.5 text-xs font-bold tracking-wide uppercase outline-none transition-all border bg-transparent border-transparent hover:border-slate-700 focus:border-blue-500 focus:bg-slate-900 cursor-pointer ${
					editingValues.taskType === 'BUG' ? 'text-red-400' :
					editingValues.taskType === 'STORY' ? 'text-green-400' :
					editingValues.taskType === 'EPIC' ? 'text-purple-400' :
					editingValues.taskType === 'SUB_TASK' ? 'text-teal-400' : 'text-blue-400'
				  }`}
				>
				  {(metadata.taskTypes || ['STORY', 'BUG', 'TASK', 'EPIC', 'SUB_TASK']).map(type => (
					<option key={type} value={type} className="bg-slate-800 text-slate-200 font-medium text-left">
			  ...
			  {editingValues.taskType !== task.taskType && !lockedFields.taskType && (
				<button onClick={() => commitFieldUpdate('taskType')} className="p-1.5 bg-green-600 text-white rounded hover:bg-green-700 shadow-sm cursor-pointer"><Check size={13} /></button>
			  )}
			</div>
		  </div>
		  ...
		  {/* FIELD SEQUENCE 6: DEVELOPMENT / GITHUB PULL REQUESTS */}
		  <div>
	  ...
	  {activeInlineMenu && (
		<div 
		  ...
		  className="fixed bg-slate-800 rounded-xl shadow-2xl border border-slate-700 p-1.5 z-50 min-w-[150px] max-w-xs animate-in fade-in zoom-in-95 duration-100 flex flex-col gap-0.5"
		  style={{
